...
const FeaturesList = ({ items, color = "green" }: FeaturesListProps): React.ReactElement => (
  <ul className="space-y-2 text-gray-700">
    {items.map((item, index) => (
      <li key={index} className="flex items-start">
        <span className={`inline-block w-2 h-2 rounded-full bg-${color}-500 mt-2 mr-2 flex-shrink-0`} />
        <span>{item}</span>
      </li>
    ))}
  </ul>
...
const ProductDetails = ({ product }: ProductDetailsProps): React.ReactElement => (
  <div className="mt-6 space-y-6">
    ...
    <div>
      ...
      <FeaturesList items={product.features} color="green" />
    </div>

    <div>
      <h4 className="text-xl font-semibold mb-3 text-gray-800">Especificações Técnicas</h4>
      <FeaturesList items={product.specs} color="blue" />
    </div>
  </div>
);
...
const QuoteButton = ({ productName }: QuoteButtonProps): React.ReactElement => {
  ...
  return (
    <motion.a
      ...
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      className="flex items-center justify-center gap-2 px-6 py-3 bg-green-500 hover:bg-green-800 text-white rounded-lg font-semibold shadow-lg transition-all"
    >
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        transition={{ duration: ANIMATION.DURATION.standard, delay: 0.2 }}
